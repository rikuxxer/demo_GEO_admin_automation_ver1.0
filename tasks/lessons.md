...
1. レポート依頼一覧のプロジェクト列が `project_id` のみ表示され、案件名が出ていなかった
2. 「更新」ボタンがデフォルトの outline 色で、他のプライマリボタン（`#5b5fff`）と不統一だった

### 今後のルール

- テーブル・一覧・詳細画面でIDを表示する際は、必ず対応する名前も表示する（`名前 (ID)` 形式）
- ボタンのアクセントカラーは `#5b5fff` で統一。shadcn/ui の `variant="outline"` をそのまま使わず、色クラスを必ず追加する
- CLAUDE.md のルール13・14に追記済み
