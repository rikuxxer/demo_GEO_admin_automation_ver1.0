# Excel入力規則とバリデーションの更新

## 📋 変更内容

半径の入力範囲を0-10000に拡張し、自由入力に対応しました。

## ✅ 完了した変更

### 1. フロントエンドの入力規則

- ✅ `PoiForm.tsx`に0-10000の自由入力フィールドを追加
- ✅ リアルタイムバリデーション（0-10000の範囲チェック）
- ✅ フォーム送信時のバリデーション
- ✅ エラーメッセージの表示

### 2. Excelテンプレートの入力規則

- ✅ `excelTemplateGenerator.ts`の配信範囲フィールドを更新
- ✅ 数値入力規則（0-10000）を設定
- ✅ テキスト形式（"500m"など）もパーサー側で処理可能

**変更前:**
```typescript
// 固定リストのみ
row.getCell(3).dataValidation = { 
  type: 'list', 
  formulae: ['"50m,100m,...,10000m"'] 
};
```

**変更後:**
```typescript
// 0-10000の範囲で自由入力可能
row.getCell(3).dataValidation = {
  type: 'whole',
  operator: 'between',
  formulae: [0, 10000],
  allowBlank: true,
  showErrorMessage: true,
  error: '0-10000の範囲で入力してください（例: 500 または 500m）',
  errorStyle: 'warning'
};
```

### 3. Excelパーサーのバリデーション

- ✅ `excelParser.ts`の配信範囲バリデーションを更新
- ✅ 数値のみ（例: "500"）と"数値m"形式（例: "500m"）の両方に対応
- ✅ 0-10000の範囲チェックを追加

**変更前:**
```typescript
if (/^\d+m$/.test(rStr)) segment.designated_radius = rStr;
else errors.push({ message: '「50m」のような形式で入力してください' });
```

**変更後:**
```typescript
const numMatch = rStr.match(/^(\d+)$/);
const mMatch = rStr.match(/^(\d+)m$/);

if (numMatch) {
  const radiusValue = parseInt(numMatch[1], 10);
  if (radiusValue >= 0 && radiusValue <= 10000) {
    segment.designated_radius = `${radiusValue}m`; // "m"を付けて保存
  } else {
    errors.push({ message: '配信範囲は0-10000の範囲で入力してください' });
  }
} else if (mMatch) {
  const radiusValue = parseInt(mMatch[1], 10);
  if (radiusValue >= 0 && radiusValue <= 10000) {
    segment.designated_radius = rStr;
  } else {
    errors.push({ message: '配信範囲は0-10000の範囲で入力してください' });
  }
} else {
  errors.push({ message: '配信範囲は0-10000の数値、または「500m」のような形式で入力してください' });
}
```

## 📊 対応状況

| 項目 | 状態 | 説明 |
|------|------|------|
| フロントエンド入力規則 | ✅ 完了 | 0-10000の自由入力フィールドを追加 |
| フロントエンドバリデーション | ✅ 完了 | 0-10000の範囲チェックを実装 |
| Excelテンプレート入力規則 | ✅ 完了 | 0-10000の数値入力規則を設定 |
| Excelパーサーバリデーション | ✅ 完了 | 0-10000の範囲チェックを実装 |
| スプレッドシート出力バリデーション | ✅ 完了 | setting_flagに応じた必須フィールドチェック |

## 🔍 テストケース

### フロントエンド

- ✅ `500` → 正常（0-10000の範囲内）
- ✅ `10000` → 正常（最大値）
- ✅ `0` → 正常（最小値）
- ❌ `10001` → エラー（範囲外）
- ❌ `-1` → エラー（範囲外）

### Excelパーサー

- ✅ `"500"` → `"500m"`として保存
- ✅ `"500m"` → `"500m"`として保存
- ✅ `"10000"` → `"10000m"`として保存
- ✅ `"10000m"` → `"10000m"`として保存
- ❌ `"10001"` → エラー（範囲外）
- ❌ `"10001m"` → エラー（範囲外）
- ❌ `"abc"` → エラー（無効な形式）

## 📝 注意事項

1. **Excelの入力規則**: 数値入力規則（0-10000）を設定していますが、テキスト形式（"500m"など）もパーサー側で処理可能です。

2. **データ形式の統一**: パーサー側で数値のみの入力（例: "500"）を"500m"形式に変換して保存します。

3. **既存データとの互換性**: 既存の"50m"形式のデータも引き続き処理可能です。

