#!/bin/bash
# タイムアウト付きで全テーブルを作成するスクリプト（改善版）

PROJECT_ID="univere-geo-demo"
DATASET_ID="universegeo_dataset"

echo "=========================================="
echo "📋 全テーブル作成（タイムアウト付き）"
echo "=========================================="
echo ""
...
# データセットの存在確認
echo "📋 データセットの存在確認..."
DATASET_FQDN="${PROJECT_ID}:${DATASET_ID}"
if timeout 10s bq show --dataset --project_id="${PROJECT_ID}" "${DATASET_ID}" >/dev/null 2>&1; then
...
create_table() {
  local TABLE=$1
  local FQTN="${PROJECT_ID}:${DATASET_ID}.${TABLE}"
  ...
  # 3) テーブルを作成
  echo "    ➡️  ${FQTN} を作成中..."
  TABLE_CREATE_OUTPUT=$(timeout 30s bq mk --table --project_id="${PROJECT_ID}" --schema="${SCHEMA_FILE}" "${FQTN}" 2>&1)
...
EOF
create_table "visit_measurement_groups"
echo ""
