...
- **デプロイ**: コミット・プッシュ後、必ず本番デプロイまで実施する。ユーザーに確認を取ったうえで `gh workflow run` でワークフローをトリガーし、完了を確認するまでタスクを完了としない。`gh` CLI が使えない場合は GitHub Actions の URL を提示してユーザーに手動トリガーを依頼する。
...
### 11. 仕様説明・対応表はコードを読んでから書く

フラグ値・カラム対応表・数値マッピングなどの仕様をユーザーに説明するとき、記憶やコンテキストだけで書いた結果、`setting_flag` の 選択半径(=4) と 自由入力半径(=2) を逆に説明するミスが発生した。
...
過去にレポート依頼一覧で `project_id` のみ表示し、ユーザーから「案件名も出して」と指摘を受けた。
- テーブル・一覧・詳細画面で ID を表示する際は、必ず対応する名前（案件名、セグメント名、グループ名など）も表示する
- 形式: `名前 (ID)` または名前のみ。IDだけの表示は禁止
- 名前の逆引きが必要な場合は、事前にマスタデータ（projects, segments 等）を取得してマッピングする

### 14. UI のボタン・アクセントカラーは `#5b5fff` に統一する
