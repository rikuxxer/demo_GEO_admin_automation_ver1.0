-- =============================================================================
-- UNIVERSEGEO BigQuery 全テーブル作成スクリプト（環境移行用）
-- =============================================================================
-- プロジェクト: universe-geo-admin-dev-12246
-- データセット: universegeo_dataset
--
-- 【必須】実行前にデータセットを作成してください。
-- エラー "Dataset was not found in location US" の対処:
--   1) データセットを US で作成する（下記コマンド）
--   2) 本SQLを BigQuery コンソールで実行（コンソールのロケーションは US のまま）
--   3) バックエンドで同じデータセットを使う場合は環境変数 BQ_LOCATION=US を設定
--
-- データセットを US で作成（推奨・コンソールが US の場合）:
--   bq mk --dataset --location=US universe-geo-admin-dev-12246:universegeo_dataset
--
-- データセットを東京で作成する場合:
--   bq mk --dataset --location=asia-northeast1 universe-geo-admin-dev-12246:universegeo_dataset
--
-- =============================================================================
...
-- 1. projects（案件）
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `universe-geo-admin-dev-12246.universegeo_dataset.projects` (
...
-- 2. segments（セグメント）
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `universe-geo-admin-dev-12246.universegeo_dataset.segments` (
...
-- 3. pois（地点情報）
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `universe-geo-admin-dev-12246.universegeo_dataset.pois` (
...
-- 4. users（ユーザー）
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `universe-geo-admin-dev-12246.universegeo_dataset.users` (
...
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `universe-geo-admin-dev-12246.universegeo_dataset.user_requests` (
  user_id STRING NOT NULL,
  name STRING,
  email STRING,
...
-- 6. messages（メッセージ）
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `universe-geo-admin-dev-12246.universegeo_dataset.messages` (
...
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `universe-geo-admin-dev-12246.universegeo_dataset.change_history` (
...
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `universe-geo-admin-dev-12246.universegeo_dataset.edit_requests` (
...
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `universe-geo-admin-dev-12246.universegeo_dataset.feature_requests` (
  request_id STRING NOT NULL,
  requested_by STRING NOT NULL,
...
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `universe-geo-admin-dev-12246.universegeo_dataset.visit_measurement_groups` (
  project_id STRING NOT NULL,
...
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `universe-geo-admin-dev-12246.universegeo_dataset.sheet_exports` (
...
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `universe-geo-admin-dev-12246.universegeo_dataset.sheet_export_data` (
...
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `universe-geo-admin-dev-12246.universegeo_dataset.report_requests` (
  request_id STRING NOT NULL,
  requested_by STRING NOT NULL,
  requested_by_name STRING NOT NULL,
  requested_at TIMESTAMP NOT NULL,
...
-- =============================================================================
-- ALTER TABLE `universe-geo-admin-dev-12246.universegeo_dataset.pois`
-- SET OPTIONS(partition_expiration_days=1095);
--
-- ALTER TABLE `universe-geo-admin-dev-12246.universegeo_dataset.segments`
-- SET OPTIONS(partition_expiration_days=1095);
--
-- ALTER TABLE `universe-geo-admin-dev-12246.universegeo_dataset.projects`
