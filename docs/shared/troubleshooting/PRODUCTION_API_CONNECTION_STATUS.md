...
成功時: 各エンドポイントが `200` などで表示され、「全 15 エンドポイントが応答しました」と出れば本番接続は問題ありません。**このテストは GET のみのため、POST で 500 になる不具合（例: セグメント作成失敗）は検知できません。**

### 3. 本番で書き込みテスト（POST の 500 検知）

セグメント作成など POST が本番で失敗していないかを確認するには、書き込みテストを実行します。

```bash
node scripts/test-api-write-endpoints.js https://universegeo-backend-i5xw76aisq-an.a.run.app
```

- 案件が1件以上ある場合、テスト用セグメントを 1 件作成し、直後に削除します。
- `POST /api/segments` が 500 の場合は「✗ 500 …」と表示され、終了コード 1 になります。

### 4. 本番で全カラム接続確認（レスポンスの必須カラム検証）
